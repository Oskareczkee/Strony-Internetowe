...
var globalWorkersAmount : number=0;
var workersSrc =[
    "Images/hehe.jpg",
...
const workerWidth : number =50;
const workerHeight : number=50;
...
function checkUpgradesAvailability(){
    ...
    let playerMoney : number = parseInt($("#money-amount").text());
...
function addWorkerToCottonfield(){
    ...
    $worker.css("top", getRandomInt(0, ($cottonfiled.height()?? 50)-workerHeight));
    $worker.css("right", getRandomInt(0, ($cottonfiled.width()??50)-workerWidth));
    $worker.appendTo($cottonfiled);
}

function calculatePrice(amuntOfUpgrades : number, basePrice: number) : number{
...
$("#click").on('click', function(){
    ...
    var money : number = parseInt($elem.text());
...
$(".upgrade-add").on('click', function(e){
    ...
    var $val : number = parseInt($(e.target).prev().text());
    var $price : number = parseInt($upgradeCostNode.text());
    var $money : number=  parseInt($moneyNode.text());
    var $basePricce : number = parseInt($upgradeCostNode.attr("data-basePrice") ?? '');
