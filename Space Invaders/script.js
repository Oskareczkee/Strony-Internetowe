...
var dropMin = 1;
var dropMax = 3;
var buildingsNum = tableCols;
var invaderSpawnChance = 0.1;
var missileRechargeTurns = 2;
var $gameTable = $("#gameTable");
var invadersCoords = [];
var missileUsageTurns = [];
var gameEnded = false;
...
function dropInvaders() {
    var invaderToDestroy = [];
    for (var x = 0; x < invadersCoords.length; x++) {
        var invaderCoords = invadersCoords[x];
        var dropRate = getRandomInt(dropMin, dropMax);
    ...
    for (var x = 0; x < invaderToDestroy.length; x++) {
        var item = invadersCoords.filter(function (i) { return i.row === invaderToDestroy[x].row && i.col === invaderToDestroy[x].col; })[0];
...
function spawnInvaders() {
    for (var x = 1; x <= tableCols; x++) {
        ...
        var roll = Math.random() <= invaderSpawnChance ? true : false;
        if (roll) {
...
function shootMissile(row) {
    ...
    for (var x = 1; x <= tableCols; x++) {
        ...
        if ($col.classList.contains("package")) {
            ...
            var item = invadersCoords.filter(function (i) { return i.row === row && i.col === x; })[0];
            var index = invadersCoords.indexOf(item);
            invadersCoords.splice(index, 1);
        }
    }
